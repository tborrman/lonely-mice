...
parser.add_argument('-c', help='Sample type (X712.PV.PFC_fdrDonut)', type=str, required=True)
...
parser.add_argument('-s', help='Sample type specific', type=bool, default=False)
...
def is_loop_in_cell(line, cell, specific, filter):
	col_id = {'X712.PV.PFC_fdrDonut': 21,
				'X931.PV.PFC_fdrDonut': 22}
	...
	if specific:
		...
		if args.f:
			...
			if qval < -1 and qval1 >= -1  and reads > 50:
				print line.strip()
				return True
			else:
				return False
...
def main():
	...
	if args.o == 'anchor':
		if args.s:
			if args.f:
				OUT = open(args.c + '_overlapping_genes_anchor_sample_specific_filter.txt', 'w' )
			else:
				OUT = open(args.c + '_overlapping_genes_anchor_sample_specific.txt', 'w' )
		else:
			OUT = open(args.c + '_overlapping_genes_anchor.txt', 'w' )
	elif args.o == 'full':
		if args.s:
			OUT = open(args.c + '_overlapping_genes_full_loop_sample_specific.txt', 'w')
		else:
			OUT = open(args.c + '_overlapping_genes_full_loop.txt', 'w')
	elif args.o == 'tss':
		if args.s:
			OUT = open(args.c + '_overlapping_genes_tss_sample_specific.txt', 'w')
		else:
			OUT = open(args.c + '_overlapping_genes_tss.txt', 'w')
	else:
